...
| **Backend** | `functions/` | Node 24, Firebase Functions v2 (onCall); single `index.js` (~5,300 lines) |
| **Rules** | `firestore.rules`, `storage.rules` | Role-based; directors see only released data |
| **Tests** | `tests/e2e/smoke.spec.ts`, `tests/e2e/release.spec.ts` | Playwright E2E; requires env vars and running app |
...
- **Auth:** Email/password; anonymous only on emulators (`DEV_FLAGS.allowAnonymousSignIn`). Director self-signup constrained by Firestore rules (allowed keys, school existence). **Appropriate for initial release.**
...
- **Largest module:** `public/modules/ui.js` is **~5,331 lines**. It holds tab/routing, auth UI, admin/director/judge-open rendering, modals, and many helpers. **Risk:** Hard to navigate and refactor; continue splitting by role or feature (e.g. `ui-admin-*`, `ui-director-*`, `ui-judge-open-*`) when touching flows.
...
- **Bundling:** No build step; ES modules loaded from `public/`. Fine for initial release; consider bundling/minification if load time or cache invalidation becomes an issue.
...
- **Smoke spec:** `tests/e2e/smoke.spec.ts` — serial smoke tests: admin (event + school), director (attach/ensemble/detach), admin (assignments), judge (workspace). Requires env: `MPA_BASE_URL`, `MPA_ADMIN_EMAIL`, `MPA_ADMIN_PASSWORD`, `MPA_JUDGE_EMAIL`, `MPA_JUDGE_PASSWORD`, `MPA_DIRECTOR_EMAIL`, `MPA_DIRECTOR_PASSWORD`.
- **Release spec:** `tests/e2e/release.spec.ts` — release-surface and director release-gate checks. Requires env: `MPA_BASE_URL`, `MPA_ADMIN_EMAIL`, `MPA_ADMIN_PASSWORD`, `MPA_DIRECTOR_EMAIL`, `MPA_DIRECTOR_PASSWORD`.
...
- **Baseline verification:** verify required env setup (`MPA_BASE_URL` and role credentials) before executing smoke/release E2E to avoid false negatives.
...
|----------|---------|----------|
| **Tests** | Legacy `#authStatus` mismatch was resolved by moving assertions to `#accountSummary` in current smoke tests. | **Resolved** |
| **Frontend** | `public/modules/ui.js` remains large (~5.3k lines); continue splitting by role/feature. | **Medium** |
| **Tests** | E2E runs fail when required `MPA_*` env vars are absent; baseline docs/scripts now document and handle this explicitly. | **Medium** |
...
1. **Keep smoke/release naming:** Maintain `tests/e2e/smoke.spec.ts` and `tests/e2e/release.spec.ts` as the baseline E2E entrypoints.
2. **Document E2E env for CI:** Keep `MPA_*` env setup documented so smoke/release suites run in GitHub Actions or locally in a repeatable way.
