export function createDirectorRegistrationRenderers({
  ...
  const registrationAutosaveTimers = new Map();
  ...
  async function renderDirectorRegistrationPanel() {
    ...
    registrationAutosaveTimers.forEach((timerId) => clearTimeout(timerId));
    registrationAutosaveTimers.clear();
    const schoolId = getDirectorSchoolId();
    ...
    if (els.directorRegistrationMeta) {
      const eventName = getEventCardLabel(event);
      ...
      const deadline = event.registrationDeadline?.toDate
        ...
        : null;
      const schoolName = els.directorSummarySchool?.textContent?.trim() || "No school attached";
      const parts = [
        eventName,
        dates,
        `School: ${schoolName}`,
      ...
      els.directorRegistrationMeta.textContent = parts.join(" \u2022 ");
    }
    ...
    if (els.directorRegistrationEnsembleList) {
      ...
      ensembles.forEach((ensemble) => {
        ...
        row.className = "row registration-ensemble-check";
      ...
      createLabel.className = "row registration-add-ensemble-row";
      ...
      selectedEnsembles.forEach((ensemble) => {
        ...
        const doUpsert = async () => {
          ...
          try {
            ...
            entryDataByEnsemble.set(ensemble.id, {
              ...(entryDataByEnsemble.get(ensemble.id) || {}),
              declaredGradeLevel: gradeSelect.value,
              declaredGradeFlex: flexCheck.checked,
              commentsOnly,
              feeWaiverRequested: waiverCheck.checked,
            ...
            alertUser(error?.message || "Could not save registration.");
          }
        };

        const scheduleUpsert = (delayMs = 300) => {
          const existing = registrationAutosaveTimers.get(ensemble.id);
          if (existing) {
            clearTimeout(existing);
          }
          const timer = window.setTimeout(() => {
            registrationAutosaveTimers.delete(ensemble.id);
            void doUpsert();
          }, delayMs);
          registrationAutosaveTimers.set(ensemble.id, timer);
        };

        gradeSelect.addEventListener("change", () => scheduleUpsert(200));
        flexCheck.addEventListener("change", () => scheduleUpsert(200));
