...
import { resolveAdminDirectorPersistPrimary } from "./director-attach-policy.js";
import { computePacketSummary } from "./judge-shared.js";
import { computeDirectorReadiness } from "./director-readiness.js";
...
export function buildDirectorAutosavePayload() {
  ...
  const normalizedLunch = {
    ...
    pickupTiming:
      lunchOrder.pickupTiming === "before" || lunchOrder.pickupTiming === "after"
        ? lunchOrder.pickupTiming
        : "",
...
export function normalizeEntryData(data, defaults) {
  ...
  base.lunchOrder.pickupTiming =
    base.lunchOrder.pickupTiming === "before" || base.lunchOrder.pickupTiming === "after"
      ? base.lunchOrder.pickupTiming
      : "";
...
export function computeDirectorCompletionState(entry) {
  const readiness = computeDirectorReadiness(entry, {
    selectedEnsembleId: state.director.selectedEnsembleId,
    hasSchool: Boolean(getDirectorSchoolId()),
    mpaCacheByGrade: state.director.mpaCacheByGrade,
  });
...
export async function saveLunchSection() {
  ...
  lunchOrder.pickupTiming =
    lunchOrder.pickupTiming === "before" || lunchOrder.pickupTiming === "after"
      ? lunchOrder.pickupTiming
      : "";
  const lunchCount = lunchOrder.pepperoniQty + lunchOrder.cheeseQty;
  if (lunchCount > 0 && !lunchOrder.pickupTiming) {
    return {
      ok: false,
      reason: "validation",
      message: "Select whether pizza is needed before or after performance time.",
    };
  }
...
export function validateEntryReady(entry) {
  return computeDirectorReadiness(entry, {
    selectedEnsembleId: state.director.selectedEnsembleId,
    hasSchool: Boolean(getDirectorSchoolId()),
    mpaCacheByGrade: state.director.mpaCacheByGrade,
  }).issues;
...
export async function attachDirectorSchool(schoolId, { persistPrimary } = {}) {
  ...
  if (state.auth.userProfile.role === "admin") {
    const shouldPersistPrimary = resolveAdminDirectorPersistPrimary(persistPrimary);
  ...
  merged.packets.forEach((packet) => {
    ...
    if (!canAssemble) {
      standaloneOpenGroups.push({
        ...
        latestAudioUrl: packet.canonicalAudioUrl || packet.latestAudioUrl || "",
        ...
        supplementalLatestAudioDurationSec: Number(packet.supplementalLatestAudioDurationSec || 0),
        judgePosition,
        assignmentEventId,
    ...
    const syntheticSubmission = {
      ...
      supplementalAudioDurationSec: Number(packet.supplementalLatestAudioDurationSec || 0),
      transcript: "",
      transcriptFull: "",
    };
