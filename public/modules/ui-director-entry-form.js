export function createDirectorEntryFormRenderers({
  ...
  function renderPercussionOptions() {
    ...
    PERCUSSION_OPTIONS.forEach((item) => {
      if (item && typeof item === "object" && item.type === "heading") {
        ...
        els.percussionOptions.appendChild(heading);
        return;
      }
      if (item && typeof item === "object" && item.type === "note") {
        const note = document.createElement("div");
        note.className = "percussion-option-note";
        if (item.parent) note.classList.add("is-nested");
        note.textContent = item.label || "";
        els.percussionOptions.appendChild(note);
        return;
      }
      if (item && typeof item === "object" && item.type === "plain") {
        const plain = document.createElement("div");
        plain.className = "percussion-option-plain";
        plain.textContent = item.label || "";
        els.percussionOptions.appendChild(plain);
        return;
      }
      const optionType = item && typeof item === "object" ? item.type : "";
      if (optionType && optionType !== "checkbox") return;
      const value =
  ...
  function renderDirectorEntryForm() {
    ...
    if (els.pizzaPickupTimingInput) {
      els.pizzaPickupTimingInput.value =
        state.director.entryDraft.lunchOrder?.pickupTiming || "";
      els.pizzaPickupTimingInput.onchange = () => {
        state.director.entryDraft.lunchOrder.pickupTiming =
          String(els.pizzaPickupTimingInput.value || "");
        applyDirectorDirty("lunch");
      };
    }
